...
const RegisterDoctor = () => {
  ...
  const validate = () => {
    const errors = {};
    const { firstName, lastName, email, address, phone, age, gender, qualification, experience, speciality, password, confirmPassword } = formData;
    ...
    if (!speciality) errors.speciality = "Medical History is required.";
    if (!qualification) errors.qualification = "Qualificationis required.";
    if (!experience) errors.speciality = "Experience is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await axiosInstance.post('/api/doctor/registerDoctor', formData);
        ...
        navigate('/admin/doctor/list'); // Redirect after successful registration
      } catch (error) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className=" bg-[#567fbf]  p-8 rounded-lg shadow-lg max-w-xl mx-auto">
      ...
      <div className='mx-5'>
          ...
          <select name='gender' value={formData.gender} onChange={handleInputChange} className='px-4 text-start rounded w-full py-2 text-black leading-tight focus:outline-none focus:shadow-outline'> 
      ...
      <div className='mx-5 mb-2'>
        ...
        <input
         ...
         className="rounded w-full py-2 px-2 text-black leading-tight focus:outline-none focus:shadow-outline"
        />
            {errors.speciality && <div className="text-red-500 text-sm">{errors.speciality}</div>}
      ...
      <div className='mx-5 mb-2'>
        ...
        <input
         ...
         className="rounded w-full py-2 ppx-2 text-black leading-tight focus:outline-none focus:shadow-outline"
        />
            {errors.qualification && <div className="text-red-500 text-sm">{errors.qualification}</div>}
      ...
      <button className='bg-pink-600 px-5 rounded-xl py-3 text-center text-white font-bold font-serif text-xl'>
        Submit
      </button>
      </div>
