  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await axiosInstance(`/api/patient/get/${userId}`);
   ...
   const handleEditClick = () => {
    const userId = localStorage.getItem("userId");
    navigate(`/patient/patient/edit/${userId}`);
  };
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#567fbf]">
      <div className="bg-white p-8 rounded-lg shadow-lg w-fit">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 mb-2">
          <strong>Medical History:</strong> {profile.profileData.medicalHistory}
          </p>
        </div>
      </div>
      <ToastContainer />
